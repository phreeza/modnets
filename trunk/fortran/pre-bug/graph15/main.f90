...
integer,parameter :: n_pop=1000,L=300,n_gen=20000,run_thresh=15,n_gates=20,n_gates_nom=11,n_transient=5,n_inp=4
real,parameter :: p_wire=1.0,p_mut=0.7
...
logical :: again = .true., MVG = .true.
integer :: old_sum,epoch=20
real :: b_wire=0.0
...
do a = 1,n_pop
   ...
   !else
      if (sum(active(n_inp+1:n_gates-1)) >= n_gates_nom) then
             qual(a) = qual(a) - 0.2*(sum(active(n_inp+1:n_gates-1))-n_gates_nom)
      end if
   !end if 
...
if (mod(gen,epoch)==0) then
    ...
    print*,gen,maxval(qual)/2.**n_inp,real(sum(qual))/(2.**n_inp*n_pop),sum(wire_list)/n_pop, &
           sum(mod_list)/n_pop,maxval(mod_list),sum(qual(find(qual==maxval(qual))))/sum(qual==maxval(qual)),run
